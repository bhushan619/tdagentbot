...
def _clean_config(payload: dict[str, Any]) -> dict:
    ...
    return {
        "mode": mode,
        "trade_amount": _as_float(data, "trade_amount", 0.01, 10000),
        "trade_duration_sec": _as_int(data, "trade_duration_sec", 5, 3600),
        "confidence_threshold": _as_float(data, "confidence_threshold", 0, 1),
        "max_consecutive_losses": _as_int(data, "max_consecutive_losses", 1, 100),
        "max_daily_loss": _as_float(data, "max_daily_loss", 0, 100000),
        "bypass_risk_in_demo": bool(data.get("bypass_risk_in_demo", True)),
        "min_candles_required": _as_int(data, "min_candles_required", 5, 5000),
        "candle_interval_sec": _as_int(data, "candle_interval_sec", 1, 300),
    }
...
def logs() -> list[dict]:
    ...
    rows = conn.execute(
        """SELECT ts, symbol, direction, amount, duration, confidence,
                  result, pnl, balance_after, mode, session_id
           FROM trades
           WHERE session_id = (SELECT session_id FROM trades
...
@app.get("/bot-logs")
def bot_logs(lines: int = 300) -> dict:
